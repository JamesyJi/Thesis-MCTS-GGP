...
public:
    ...
    std::pair<Common::Result, int> Simulate(NodeT& node)
    {
        ...
        return std::make_pair(simulateState.EvaluateState(move), depth);
    }

    void BackPropagate(NodeT& node, Common::Result result, int depth);
...
template <typename M, typename TTraits>
void Model<M, TTraits>::BackPropagate(NodeT& node, Common::Result result, int depth)
{
    ...
    while (curNode != nullptr)
    {
        ...
        // The depth is the depth from the child node. We will +1 after each update as the parent node is one level higher...
        curNode->VisitAndAvgDepth(depth++);
        curNode = curNode->GetParent();
    }
}
